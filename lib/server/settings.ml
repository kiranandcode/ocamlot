...
let handle_settings_post req =
  ...
  | Some user when user.is_admin ->
    ...
    | Ok registration_allowed ->
      let* _ = sql req (Database.Admin.set_registration_allowed registration_allowed) in
      redirect req "/settings"
    end
  | _ ->
    redirect req "/feed"
...
let route =
  Dream.scope "/settings" [] [
    Dream.get "" @@ Error_handling.handle_error_html @@ handle_settings_get;
    Dream.post "" @@ Error_handling.handle_error_html @@ handle_settings_post;
  ]
