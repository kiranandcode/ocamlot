...
let of_local (local: Local_user.t) (module DB: DB) =
  let* result = DB.find_opt lookup_local_id_request local.id |> flatten_error in
  match result with
  | Some id -> R.return (id, resolve)
  | None ->
    let* () = DB.exec create_local_user_request local.id |> flatten_error in
    let* id = DB.find lookup_local_id_request local.id |> flatten_error in
    R.return (id, resolve)

let of_remote (remote: Remote_user.t) (module DB: DB) =
  let* result = DB.find_opt lookup_remote_id_request remote.id |> flatten_error in
  match result with
  | Some id -> R.return (id, resolve)
  | None ->
    let* () = DB.exec create_remote_user_request remote.id |> flatten_error in
    let* id = DB.find lookup_remote_id_request remote.id |> flatten_error in
    R.return (id, resolve)
