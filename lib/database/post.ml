...
let create_post_request =
  Caqti_request.exec ~oneshot:false
    T.Std.(tup4 (option string) string int64
             (tup4 bool (option string) string
                (tup2 timestamp (option string))))
 {|
INSERT OR IGNORE
...
let insert_post_to_request =
  Caqti_request.exec ~oneshot:false T.Std.(tup2 int64 int64) {|
INSERT OR IGNORE
...
let collect_post_to_request =
  Caqti_request.collect ~oneshot:false T.Std.int64 T.Std.int64  {|
SELECT actor_id
From PostTo
WHERE post_id = ?
|}
...
let add_post_tag_request =
  Caqti_request.exec ~oneshot:false T.Std.(tup3 int64 int64 (option string)) {|
INSERT OR IGNORE
...
let collect_post_tags_request =
  Caqti_request.collect ~oneshot:false
    T.Std.int64
    T.Std.(tup3 int64 string (option string)) {|
