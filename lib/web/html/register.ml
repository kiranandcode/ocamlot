  ...
  H.body [
    H.div ~a:[H.a_class ["hero"]] [
      H.div ~a:[H.a_class ["container"; "hero-body"]] @@ List.concat [
        [H.h1 ~a:[H.a_class ["title"]] [H.txt "Register an account on OCamlot"]];
        (List.map (fun err -> notification ~classes:["is-danger"; "is-light"] [H.txt err]) errors);
        [H.form ~a:[H.a_action "/register"; H.a_method `Post] [
          Utils.csrf_tag req;
          field [
          ...
          field [
            label ~for_:"password" "Password (again):";
            control [ password_input ~name:"password" () ]
          ];
          field [ control [
            H.input ~a:[H.a_class ["button"; "is-link"];  H.a_value "Sign-up"; H.a_input_type `Submit] ();
            H.a ~a:[H.a_class ["button"; "is-light"]; H.a_href "/login"] [H.txt "Login"]
          ] ]
        ]];
      ]
    ];
